...
function Gallery() {
    ...
    const deleteImage = (imageFullURL) => {
        console.warn(imageFullURL)
        Axios.get(apiBaseUrl + 'api/v1/remove-image/')
            .then((res) => {
                setModel(false)
                console.log(res)
    ...
    return (
        ...
        <div className={model? 'model open': 'model'}>
            <img src={temPicSrc} alt="" />
            <HighlightOffIcon onClick={()=> setModel(false)}/>
            {/* <DeleteForeverIcon onClick={()=> deleteImage()}/> */}
            {/* <ul>
                <li><HighlightOffIcon/></li>
                <li><DeleteForeverIcon/></li>  
            </ul> */}
           
        </div>
        <div className="gallery">
        {loading ? (<h1>Loading...</h1>) : error ? (<h1>Server Error...</h1>) : (data.map((image, i) => {
                    return (
                        <div className="pics" key={i} onClick={()=> getImg(image.imageFullURL)} >
                       <img src={image.imageFullURL} alt="" style={{width:'100%'}}/>
                        </div>
                    )
                }))}
